...
适用版本：`0.1.0-beta.3`。显式指定版本或 `@beta`；`latest` 不作为当前推荐入口，发行结果以同版本 Release 为准。
...
```bash
npx @zerozhang-giza/kdl-agent@0.1.0-beta.3 install
```

Agent 协助安装时指定目标工具，并把凭证输入留给用户本地终端：

```bash
npx @zerozhang-giza/kdl-agent@0.1.0-beta.3 install --yes --agent codex --no-login
```
...
只安装 npm CLI：`npm install -g @zerozhang-giza/kdl-agent@0.1.0-beta.3`。

Skill 可以单独安装；下面示例仅选 Codex，可换为实际工具：

```bash
npx --yes skills@1.5.25 add https://github.com/gizaZerozhang/kdl-agent-cli/tree/v0.1.0-beta.3/skills/kdl-agent --global --skill kdl-agent --agent codex --yes
```

无 Node.js 的环境可下载 [GitHub Release](https://github.com/gizaZerozhang/kdl-agent-cli/releases/tag/v0.1.0-beta.3) 的对应原生包，核对 SHA256SUMS 后解压并放入用户 PATH；执行文件为 `kdl-agent` 或 `kdl-agent.exe`。
