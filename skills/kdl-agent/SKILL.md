...
本 Skill 使用客户侧 `kdl-agent` 操作快代理已有服务。适用版本为 0.1.0；可用产物与验收状态以同版本 Release 说明为准。macOS 产物暂未签名/公证，不承诺未经实测的最低系统兼容范围。

## 使用前

1. 检查 `kdl-agent --version`、任务相关命令的 `--help`，核对官方安装来源提供的适用版本。CLI 不存在时读取 `https://github.com/kuaidaili/kdl-agent-cli/blob/v0.1.0/docs/install.md` 的同版本说明，使用其中已发布的 npm 包及固定版本；资源不可用或平台不支持时报告缺项，不猜测命令。
