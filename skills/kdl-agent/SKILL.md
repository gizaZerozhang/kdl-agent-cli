---
name: kdl-agent
description: 在用户需要查询快代理账户、余额、代理订单、接入指引、产品规格与报价、站内信，或通过快代理创建待付款订单、提交工单、获取订单密钥、提取代理及设置订单白名单时使用。通过已安装的 kdl-agent CLI 执行；不用于一般网页抓取、其他代理供应商、自动付款或管理本仓库部署。
---

# 快代理 CLI

本 Skill 使用客户侧 `kdl-agent` 操作快代理已有服务。适用版本为 0.1.0-beta.1，属于预发行验证版本；最低系统与真实业务验收状态以同版本 Release 说明为准。

## 使用前

1. 检查 `kdl-agent --version`、任务相关命令的 `--help`，核对官方安装来源提供的适用版本。CLI 不存在时读取 `https://github.com/gizaZerozhang/kdl-agent-cli/blob/v0.1.0-beta.1/docs/install.md` 的同版本说明，使用其中已发布的 npm 包及固定版本；资源不可用或平台不支持时报告缺项，不猜测命令。
