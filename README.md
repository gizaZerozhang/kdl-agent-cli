...
需要 Node.js 22.14+ 和用户可写的 npm 全局目录。配套版本 `0.1.0-beta.3` 的发行结果见 [npm](https://www.npmjs.com/package/@zerozhang-giza/kdl-agent/v/0.1.0-beta.3) 和 [GitHub Release](https://github.com/gizaZerozhang/kdl-agent-cli/releases/tag/v0.1.0-beta.3)：

```bash
npx @zerozhang-giza/kdl-agent@0.1.0-beta.3 install
```

向导安装 CLI 与同版本 Skill，然后引导本机隐藏输入凭证并验证首次只读查询。Agent 自动安装使用 `install --yes --agent codex --no-login`，由用户在本地终端完成登录。仅安装 CLI 可执行 `npm install -g @zerozhang-giza/kdl-agent@0.1.0-beta.3`。
